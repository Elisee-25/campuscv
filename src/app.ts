...
import {
  dbCreate, dbGetAll, dbGetOne, dbUpdate,
  dbDelete, dbDeleteAll, dbCount, dbExport, dbImport,
} from './db';

import {
  $, qs, toast, avColor, initials,
  statusLabel, statusClass, formatDate,
  parseSecs, youtubeEmbed, skillArr,
} from './utils';

import type {
  Profile, ProfileWithId, PageId, SortType,
  MediaTab, MediaType, StatusType,
  Experience, Education, Project, Language, Chapter,
} from './types';

// ─── NAVIGATION ──────────────────────────────────────────────

let currentPage: PageId = 'home';
let prevPage: PageId = 'members';
...
function navigateTo(page: PageId): void {
  ...
  const actions: Partial<Record<PageId, () => void>> = {
    home: renderHome,
    members: renderMembers,
    add: resetForm,
    search: () => {
      ($<HTMLInputElement>('search-q')).value = '';
      renderSearch([]);
    },
...
document.querySelectorAll('.nav-item').forEach((el) => {
  el.addEventListener('click', (e: Event) => {
    e.preventDefault();
    const page = (el as HTMLElement).dataset['page'] as PageId;
    navigateTo(page);
  });
});
...
async function renderHome(): Promise<void> {
  ...
  $('sc-schools').textContent = String(
    new Set(all.map((p) => p.school).filter(Boolean)).size,
  );
  $('sc-promos').textContent = String(
    new Set(all.map((p) => p.promo).filter(Boolean)).size,
  );
...
async function renderMembers(): Promise<void> {
  ...
  const promos = [...new Set(all.map((p) => p.promo).filter(Boolean))].sort() as string[];
  const sel = $<HTMLSelectElement>('filter-promo');
  ...
  const sort = $<HTMLSelectElement>('filter-sort').value as SortType;
  ...
  else if (sort === 'school') list.sort((a, b) => (a.school || '').localeCompare(b.school || ''));
...
async function doSearch(): Promise<void> {
  const q = $<HTMLInputElement>('search-q').value.toLowerCase().trim();
  const all = await dbGetAll();
  if (!q) return renderSearch(all);
...
function renderSearch(list: ProfileWithId[]): void {
...
function cardHTML(p: ProfileWithId): string {
  ...
  let mediaBadge = '';
  if (p.mediaType === 'video') mediaBadge = '<span class="media-badge badge-video">▶ Vidéo</span>';
  else if (p.mediaType === 'audio') mediaBadge = '<span class="media-badge badge-audio">🎵 Audio</span>';
  else if (p.mediaType === 'yt') mediaBadge = '<span class="media-badge badge-yt">▶ YouTube</span>';
...
function attachCards(): void {
  document.querySelectorAll('.mcard').forEach((c) => {
    c.addEventListener('click', () => openProfile(Number((c as HTMLElement).dataset['id'])));
  });
}

// ─── PROFILE DETAIL ──────────────────────────────────────────

async function openProfile(id: number): Promise<void> {
...
function buildProfileHTML(p: ProfileWithId): string {
  const ini = initials(p.firstname, p.lastname);
  const av = avColor(ini);
  ...
  const tlHTML = (arr: Array<Experience | Education | Project>): string =>
    arr
      .map((e) => {
        const org = (e as Experience).org ?? (e as Education).school ?? (e as Project).name ?? '';
        const role = (e as Experience).role ?? (e as Education).degree ?? (e as Project).stack ?? '';
        const period = (e as Experience).period ?? (e as Education).year ?? (e as Project).url ?? '';
        const desc = (e as Experience).desc ?? (e as Project).desc ?? '';
        return `
        <div class="tl-item">
...
function initSync(p: ProfileWithId): void {
  const player = document.getElementById('profile-player') as HTMLVideoElement | HTMLAudioElement | null;
  if (!player) return;
  ...
  player.addEventListener('timeupdate', () => {
    ...
    for (let i = 0; i < times.length; i++) {
      if (t >= (times[i] ?? 0)) idx = i;
    }
    const items = document.querySelectorAll<HTMLElement>('.chapter-item');
    items.forEach((it, i) => it.classList.toggle('active', i === idx));
    items[idx]?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  });
}

function seekChapter(el: HTMLElement, secs: number): void {
  ...
  const yt = document.getElementById('profile-yt-frame') as HTMLIFrameElement | null;
  if (yt) {
    try {
...
function resetForm(): void {
  ($<HTMLHeadingElement>('form-page-title')).textContent = 'Nouveau profil';
  (
    [
  ...
  $<HTMLSelectElement>('f-status').value = 'stage';
  $<HTMLInputElement>('f-media-type').value = '';
  $<HTMLInputElement>('f-media-data').value = '';
  const prev = $<HTMLImageElement>('photo-prev');
...
async function editProfile(id: number): Promise<void> {
  navigateTo('add');
  const p = await dbGetOne(id);
  if (!p) return;
  $('form-page-title').textContent = 'Modifier le profil';
  ($<HTMLInputElement>('f-id')).value = String(p.id);
  ($<HTMLInputElement>('f-firstname')).value = p.firstname || '';
  ($<HTMLInputElement>('f-lastname')).value = p.lastname || '';
  ($<HTMLInputElement>('f-specialty')).value = p.specialty || '';
  ($<HTMLInputElement>('f-school')).value = p.school || '';
  ($<HTMLInputElement>('f-promo')).value = p.promo || '';
  ($<HTMLInputElement>('f-city')).value = p.city || '';
  ($<HTMLTextAreaElement>('f-bio')).value = p.bio || '';
  ($<HTMLInputElement>('f-skills')).value = p.skills || '';
  ($<HTMLInputElement>('f-goal')).value = p.goal || '';
  ($<HTMLInputElement>('f-email')).value = p.email || '';
  ($<HTMLInputElement>('f-linkedin')).value = p.linkedin || '';
  ($<HTMLInputElement>('f-github')).value = p.github || '';
  ($<HTMLSelectElement>('f-status')).value = p.status || 'stage';
  ($<HTMLInputElement>('f-media-type')).value = p.mediaType || '';
  ($<HTMLInputElement>('f-media-data')).value = p.mediaData || '';
  ...
  if (p.mediaType && p.mediaData) showMediaPreview(p.mediaType, p.mediaData);

  $('exp-list').innerHTML = '';
  _expN = 0;
  (p.experiences ?? []).forEach((e) => addExpRow(e));
  if (!p.experiences?.length) addExpRow();

  $('edu-list').innerHTML = '';
  _eduN = 0;
  (p.education ?? []).forEach((e) => addEduRow(e));
  if (!p.education?.length) addEduRow();
  ...
  if (!p.languages?.length) addLangRow();
...
function addExpRow(d: Partial<Experience> = {}): void {
  const i = ++_expN;
  $('exp-list').insertAdjacentHTML(
    'beforeend',
    `<div class="drow" id="exp-${i}">
...
function addEduRow(d: Partial<Education> = {}): void {
  const i = ++_eduN;
  $('edu-list').insertAdjacentHTML(
    'beforeend',
    `<div class="drow" id="edu-${i}">
      <button class="drow-remove" onclick="this.closest('.drow').remove()">✕</button>
      <div class="g2">
...
function addProjRow(d: Partial<Project> = {}): void {
  const i = ++_projN;
  $('proj-list').insertAdjacentHTML(
    'beforeend',
    `<div class="drow" id="proj-${i}">
      <button class="drow-remove" onclick="this.closest('.drow').remove()">✕</button>
      <div class="g2">
...
function addLangRow(d: Partial<Language> = {}): void {
  const i = ++_langN;
  $('lang-list').insertAdjacentHTML(
    'beforeend',
    `<div class="drow" id="lang-${i}">
      <button class="drow-remove" onclick="this.closest('.drow').remove()">✕</button>
      <div class="g2">
...
function addChapterRow(d: Partial<Chapter> = {}): void {
  const i = ++_chN;
  $('chapters-list').insertAdjacentHTML(
    'beforeend',
    `<div class="chapter-row" id="ch-${i}">
...
function collectRows<T>(
  ...
  return [...$<HTMLElement>(listId).querySelectorAll<HTMLElement>('.drow')]
    .map((row) => {
      const id = row.id.replace(`${prefix}-`, '');
      const obj: Record<string, string> = {};
      fields.forEach((f) => {
        const el = document.getElementById(f.id.replace('N', id)) as HTMLInputElement | null;
        obj[f.key] = el ? el.value.trim() : '';
      });
      return obj;
    })
    .filter((o) => Object.values(o).some((v) => v)) as unknown as T[];
}
...
function collectChapters(): Chapter[] {
  ...
  for (let i = 1; i <= _chN; i++) {
    ...
    if (row) {
      const t = (document.getElementById(`ch-time-${i}`) as HTMLInputElement | null)?.value?.trim() ?? '';
      const ti = (document.getElementById(`ch-title-${i}`) as HTMLInputElement | null)?.value?.trim() ?? '';
      const d = (document.getElementById(`ch-desc-${i}`) as HTMLInputElement | null)?.value?.trim() ?? '';
      if (t || ti) out.push({ time: t, title: ti, desc: d });
    }
  }
  return out;
}
...
async function saveProfile(): Promise<void> {
  const first = $<HTMLInputElement>('f-firstname').value.trim();
  const last  = $<HTMLInputElement>('f-lastname').value.trim();
  const spec  = $<HTMLInputElement>('f-specialty').value.trim();
  ...
  const photoEl = $<HTMLImageElement>('photo-prev');
  ...
  const profile: Profile = {
    ...
    lastname: last,
    specialty: spec,
    school: $<HTMLInputElement>('f-school').value.trim(),
    promo: $<HTMLInputElement>('f-promo').value.trim(),
    city: $<HTMLInputElement>('f-city').value.trim(),
    bio: $<HTMLTextAreaElement>('f-bio').value.trim(),
    skills: $<HTMLInputElement>('f-skills').value.trim(),
    goal: $<HTMLInputElement>('f-goal').value.trim(),
    email: $<HTMLInputElement>('f-email').value.trim(),
    linkedin: $<HTMLInputElement>('f-linkedin').value.trim(),
    github: $<HTMLInputElement>('f-github').value.trim(),
    status: $<HTMLSelectElement>('f-status').value as StatusType,
    photo,
    mediaType: $<HTMLInputElement>('f-media-type').value as MediaType,
    mediaData: $<HTMLInputElement>('f-media-data').value,
    experiences: collectRows<Experience>('exp-list', 'exp', [
      { id: 'ex-org-N', key: 'org' },
    ...
    education: collectRows<Education>('edu-list', 'edu', [
      { id: 'ed-school-N', key: 'school' },
      { id: 'ed-degree-N', key: 'degree' },
      { id: 'ed-year-N', key: 'year' },
    ]),
    projects: collectRows<Project>('proj-list', 'proj', [
    ...
    languages: collectRows<Language>('lang-list', 'lang', [
  ...
  try {
    const existId = $<HTMLInputElement>('f-id').value;
    if (existId) {
      const updated: ProfileWithId = { ...profile, id: Number(existId) };
    ...
    console.error(err);
    showMsg(`❌ Erreur : ${(err as Error).message}`, 'err');
  }
}

function showMsg(msg: string, type: 'ok' | 'err'): void {
  const el = $('form-msg');
  el.textContent = msg;
...
function handlePhoto(e: Event): void {
  ...
  r.onload = (ev: ProgressEvent<FileReader>) => {
    const prev = $<HTMLImageElement>('photo-prev');
    prev.src = ev.target?.result as string;
    prev.style.display = 'block';
    $('photo-hint').style.display = 'none';
  };
...
function switchTab(tab: MediaTab): void {
...
function handleMediaDrop(e: DragEvent): void {
  e.preventDefault();
...
function handleMediaFile(e: Event): void {
  const file = (e.target as HTMLInputElement).files?.[0];
  if (file) processMediaFile(file);
}

function processMediaFile(file: File): void {
  ...
  r.onload = (ev: ProgressEvent<FileReader>) => {
    ...
    $<HTMLInputElement>('f-media-type').value = type;
    $<HTMLInputElement>('f-media-data').value = data;
    showMediaPreview(type, data);
    $('chapters-card').style.display = '';
    if ($('chapters-list').children.length === 0) addChapterRow();
...
function previewUrl(): void {
  const url = $<HTMLInputElement>('f-media-url').value.trim();
  const embed = youtubeEmbed(url);
  if (!embed) {
    toast('URL YouTube ou Vimeo invalide', 'err');
    return;
  }
  $<HTMLInputElement>('f-media-type').value = 'yt';
  $<HTMLInputElement>('f-media-data').value = embed;
  showMediaPreview('yt', embed);
  $('chapters-card').style.display = '';
  if ($('chapters-list').children.length === 0) addChapterRow();
}

function showMediaPreview(type: MediaType, data: string): void {
...
function clearMedia(): void {
  ...
  $('chapters-list').innerHTML = '';
  _chN = 0;
  const fi = document.getElementById('f-media-file') as HTMLInputElement | null;
  if (fi) fi.value = '';
}
...
async function importDB(e: Event): Promise<void> {
  const file = (e.target as HTMLInputElement).files?.[0];
  if (!file) return;
  try {
...
async function clearAll(): Promise<void> {
  if (!confirm('Supprimer TOUS les profils ? Action irréversible.')) return;
  await dbDeleteAll();
  toast('🗑 Base vidée');
  renderSettings();
}
...
async function seedDemo(): Promise<void> {
  ...
  if (n > 0) return;
  for (const p of DEMO) await dbCreate({ ...p });
}

// ─── GLOBAL API (pour les onclick inline dans le HTML) ───────
...
declare global {
  interface Window {
    __app: {
      ...
      renderMembers: typeof renderMembers;
      renderSettings: typeof renderSettings;
      exportDB: typeof exportDB;
      importDB: typeof importDB;
      clearAll: typeof clearAll;
      handlePhoto: typeof handlePhoto;
      handleMediaFile: typeof handleMediaFile;
      handleMediaDrop: typeof handleMediaDrop;
      previewUrl: typeof previewUrl;
      clearMedia: typeof clearMedia;
      switchTab: typeof switchTab;
      addExpRow: typeof addExpRow;
      addEduRow: typeof addEduRow;
      addProjRow: typeof addProjRow;
      addLangRow: typeof addLangRow;
      addChapterRow: typeof addChapterRow;
      seekChapter: typeof seekChapter;
